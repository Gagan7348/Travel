...
export default function TripMap({ trip }) {
  ...
  useEffect(() => {
    ...
    if (allLocations.length > 0) {
      ...
      const mapCenter = calculateCenter(allLocations);
  ...
  const onMapLoad = useCallback(() => {
    ...
    // For Google Maps, we can get bounds differently
    // const bounds = mapRef.current.getBounds();
    // if (bounds) {
    //   // Convert to array format if needed
    //   // setBounds(bounds.toArray().flat());
    // }
  }, []);
  ...
  useEffect(() => {
    const generateSampleEssentialsWithBetterData = () => {
      if (!mapRef.current || !mapRef.current.state || !mapRef.current.state.map) return;
  ...
  // Prepare points for clustering (simplified for Google Maps)
  ...
  const filterByDay = (day) => {
    ...
    if (dayLocations.length > 0) {
      const mapCenter = calculateCenter(dayLocations);
  ...
  // Handle marker click for zooming (simplified for Google Maps)
            ...
            <div style={{ height: '500px', position: 'relative' }}>
              <LoadScript googleMapsApiKey={import.meta.env.VITE_GOOGLE_PLACE_API_KEY}>
                <GoogleMap
                  ...
                  onZoomChanged={() => {
                    if (mapRef.current && mapRef.current.state && mapRef.current.state.map) {
                      setZoom(mapRef.current.state.map.getZoom());
                    }
                  }}
                >
                {/* Render markers */}
                <AnimatePresence>
                  {points.map(point => {
                    const dayIndex = (point.day - 1) % dayColors.length;
                    ...
                    return (
                      <Marker
                        ...
                        onClick={() => handleMarkerClick(point.location)}
                      >
                        <MapMarker
                          item={point.location}
                ...
                {/* Manually render nearby essentials as markers */}
                {showEssentials && nearbyEssentials.length > 0 && (
                  <AnimatePresence>
